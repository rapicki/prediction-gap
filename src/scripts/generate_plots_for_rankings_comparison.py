...
def create_plot(results_path: Path, model_name: str, pgi_name: str, k: str):
    ...
    exact_results = {ranking_name: np.mean(np.load(results_path / model_name / "pgi" / ranking_name / f"{pgi_name}_{k}.npy"))
                     for ranking_name in exact_ranking_names}
    shap_result = np.mean(np.load(results_path / model_name / "pgi" / f"{name}_shap_ranking" / f"{pgi_name}_{k}.npy"))

    plt.figure()
    plt.plot(stddevs, exact_results.values(), label="PG rankings")
    plt.plot(stddevs, [shap_result] * len(stddevs), linestyle="dashed", label="SHAP rankings")
    ...
    plt.savefig(Path("plots") / f"{model_name}_{pgi_name}_{k}.pdf")
    plt.close()
